<?php
	...
	class DelMark implements Keyboard {
		...
		public function __construct(Api $api, array $object, array $payload) {
			...
			$data = json_decode(R::findOne('cache', 'WHERE `user_id` = ? AND `name` = ?', [ $object['user_id'], 'schedule-'.date('d.m.Y') ])['data'], true);
			...
			if($db == null) {
				...
				$vkApi->editMessage("⚙️ Выберите пары на которых хотите отметиться:", $payload['update'], $object['peer_id'], Marking::getKeyboardOrCarousel($type, $data, $object, $payload['update']));
				return false;
