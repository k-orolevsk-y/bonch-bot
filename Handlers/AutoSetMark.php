	...
	class AutoSetMark {
		...
		private function start() {
			...
			foreach($this->schedule as $item) {
				...
				if($marking['status'] == 0) {
					if($item['status'] == 2) {
						$vkApi->sendMessage("⚙️ Отметиться на паре $schedule_name не удалось, будут ещё попытки отметиться до конца пары, если не получится, я пришлю об этом сообщение в диалог.", [
							'peer_id' => $item['user_id'], 'forward' => []
						]);
					}
