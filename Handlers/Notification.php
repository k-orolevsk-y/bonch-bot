	...
	class Notification {
		...
		private function start() {
			...
			foreach($this->users as $user) {
				...
				$schedule = json_decode(exec("python3.9 ../Python/GetSchedule.py $login $pass"), true);
				if($schedule == null) continue;
				elseif($schedule['count'] < 1) continue;

				$cache = R::findOne('cache', 'WHERE `user_id` = ? AND `name` = ?', [ $user['user_id'], "schedule-".date('d.m.Y') ]);
				if($cache == null) {
					$cache = R::dispense('cache');
					$cache['user_id'] = $user['user_id'];
					$cache['name'] = "schedule-".date('d.m.Y');
				...
				$vkApi->sendMessage("👋🏻 Доброе утро.\n📝 По расписанию у Вас сегодня " . $this->api->pluralForm($schedule['count'], [ 'пара', 'пары', 'пар' ]) . ".\n⚙️ На каких Вас отметить?\n\n🔕 Рассылку о парах можно отключить в профиле.", [
					...
					'forward' => []
				] + Marking::getKeyboardOrCarousel($type, $schedule, [ 'from_id' => $user['user_id'] ], 0));
			}
		}
